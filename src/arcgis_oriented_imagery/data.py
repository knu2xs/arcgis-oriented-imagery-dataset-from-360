...
def create_file_geodatabase(file_geodatabase_path: Union[str, Path]) -> Path:
    ...
    file_geodatabase_path = Path(file_geodatabase_path) if not isinstance(file_geodatabase_path, Path) else file_geodatabase_path
    ...
    if not file_geodatabase_path.exists():
        arcpy.management.CreateFileGDB(str(file_geodatabase_path.parent), file_geodatabase_path.name)
...
def create_oriented_imagery_dataset(
    ...
    if isinstance(spatial_reference, int):
        ...
        raise TypeError("The spatial_reference parameter must be an integer EPSG code or an arcpy SpatialReference object.")
...
def add_images_to_oriented_imagery_dataset(
    ...
    imagery_category: Optional[Literal['Horizontal', 'Oblique', 'Nadir', '360', 'Inspection']] = '360',
    ...
    dataset_path = Path(dataset_path) if not isinstance(dataset_path, Path) else dataset_path
    images_path = Path(images_path) if not isinstance(images_path, Path) else images_path
    if camera_info_table:
        camera_info_table = Path(camera_info_table) if not isinstance(camera_info_table, Path) else camera_info_table

    # ensure the oriented imagery dataset exists
    if not dataset_path.exists():
        raise FileNotFoundError(f"The specified Oriented Imagery Dataset does not exist: {dataset_path}")

    # ensure the images path exists and is a directory
    if not images_path.exists() or not images_path.is_dir():
        raise FileNotFoundError(f"The specified images path does not exist or is not a directory: {images_path}")
    
    # if the camera info table is provided, ensure it exists
    if camera_info_table and not camera_info_table.exists():
        raise FileNotFoundError(f"The specified camera info table does not exist: {camera_info_table}")
    ...
    if camera_info_field_map is not None:
        ...
        from .schema import rename_csv_columns  # delayed import to avoid circular dependency
...
def process_camera_info_table(
    ...
    # ensure paths are Path objects
    camera_info_table = Path(camera_info_table) if not isinstance(camera_info_table, Path) else camera_info_table
    working_directory = Path(working_directory) if not isinstance(working_directory, Path) else working_directory
    ...
    if output_dataset is None:
        ...
        output_dataset = working_directory / f"{base_name}.gdb" / f"{base_name}_oriented_imagery"

    # ensure the output dataset path is a Path object
    output_dataset = Path(output_dataset) if not isinstance(output_dataset, Path) else output_dataset
...
def get_new_camera_info_tables(
    ...
    local_working_directory = Path(local_working_directory) if not isinstance(local_working_directory, Path) else local_working_directory
    ...
    prefix = re.sub(r's3://[^/]+/', '', s3_bucket_path) # remove 's3://bucket-name/' to get the prefix
    new_tables = []
    ...
    try:
        ...
        for obj in response['Contents']:
            ...
            if filename.endswith('.csv') and ((filename not in existing_manifest) or (existing_manifest[filename] != last_modified)):
                # download the file
        ...
        logger.error("AWS credentials not found or incomplete. Please configure your AWS credentials.")
