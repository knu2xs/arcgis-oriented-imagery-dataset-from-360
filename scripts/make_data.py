...
logger = arcgis_oriented_imagery.utils.get_logger(
    level=log_level, logfile_path=log_dir / f"make_data_{timestamp}.log"
)
...
new_csv_files = arcgis_oriented_imagery.data.get_new_camera_info_tables(
    s3_bucket_path=s3_bucket, local_working_directory=working_directory
)
...
if new_datasets:
    logger.info(
        f"{len(new_datasets):,} new oriented imagery datasets created: {[str(d) for d in new_datasets]}"
    )
